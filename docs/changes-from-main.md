...
- Adjustment, cancellation, proxy, batch, mapping, AG Grid and DataFrame
...
| File | Status | Change |
|---|---|---|
| `.env.example` | Modified | Replaces multiple storage-mode variables with one `DATABASE_URL`; documents the project key, mapping manifest, failure injection, audit actor and API port 8001. |
| `README.md` | Expanded | Adds architecture, setup, workflows, mappings, batch selection, reconciliation, DataFrame calculations, the semantic dictionary, testing and developer onboarding. Removes obsolete storage modes and authentication. |
| `docs/api-testing-guide.md` | Added | Documents the API routes with Swagger and curl examples, request bodies and expected workflows. |
...
| File | Status | Change |
|---|---|---|
| `backend/app/auth.py` | Removed | The temporary mock SSO, session and role implementation is no longer part of this branch. |
| `backend/app/main.py` | Modified | Adds FO-system lookup, batch filtering, mapping routes, cancellation, proxy, batch preview/commit, history, register, revert and reconciliation. It also provides structured configuration and database errors. |
...
| `backend/tests/test_api_routes.py` | Added | Public routes, absence of authentication routes and structured configuration/database errors. |
| `backend/tests/test_auth.py` | Removed | Authentication-specific tests were deleted with the feature. |
...
| `frontend/src/main.tsx` | Modified | Starts the application directly inside the React Query provider. |
| `frontend/src/AuthGate.tsx` | Removed | The login/session gate is no longer part of the application. |
| `frontend/src/auth.css` | Removed | Authentication-specific styles were deleted. |
...
| `frontend/src/api.ts` | Modified | Adds typed calls for FO systems, batch filtering, mappings, cancellation, proxy, history, register, revert and reconciliation. |
| `frontend/src/types.ts` | Modified | Adds lineage, proxy, cancellation, batch, mapping and history types. |
...
3. Parquet-based mapping assistance.
4. Cancelled-trade visibility and mapping UI refinement.
5. API and communication documentation.
6. AG Grid search and FO-system batch selection.
7. DataFrame enrichment functions.
8. Central semantic field dictionary.
9. Removal of every storage mode except `vertica_sim` + `adjustment_meta`.
10. Removal of the temporary mock authentication and role layer.
